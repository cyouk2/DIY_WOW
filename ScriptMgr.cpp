...
// cyouk2
bool ScriptMgr::OnGossipSelectItem(Player* player, Item* item, uint32 sender, uint32 action, SpellCastTargets const& targets)
{
    ASSERT(player);
    ASSERT(item);
    //cyouk2
    player->PlayerTalkClass->ClearMenus();
    GET_SCRIPT_RET(ItemScript, item->GetScriptId(), tmpscript, false);
    return tmpscript->OnGossipSelect(player, item, sender, action, targets);
}
